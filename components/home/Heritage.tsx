...
export default function Heritage() {
  return (
    <section className="py-20 px-4 bg-[#111111] border-b border-[#2A2A2A]">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        ...
        <div>
          ...
          <h2
            className="font-black uppercase text-white leading-tight mb-10"
            ...
            {timeline.map((item, i) => (
              <div key={item.year} className="flex gap-5">
                ...
                <div className="flex flex-col items-center flex-shrink-0">
                  <div className="w-12 h-12 rounded-full bg-[#C8281E] flex items-center justify-center shadow-lg shadow-red-900/30">
                    <span className="text-white text-[11px] font-black leading-tight text-center">
                      {item.year}
                    </span>
                  </div>
                  {i < timeline.length - 1 && (
                    <div className="w-px flex-1 bg-[#2A2A2A] min-h-[3rem]" />
                  )}
                </div>
                {/* Text */}
                <div className="pb-10">
        ...
        <div className="lg:sticky lg:top-24">
          <div className="bg-[#0D0D0D] border border-[#2A2A2A] rounded-2xl p-8 sm:p-10">
            <p className="text-gray-600 text-[10px] uppercase tracking-widest mb-5">
              Misión — Gold Eagle Co.
            </p>
            <blockquote className="font-bold text-white leading-snug mb-8"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {pillars.map((p) => (
                <div key={p.label} className="flex items-start gap-3">
                  ...
                  <span className="text-sm text-gray-400 leading-snug">{p.label}</span>
                </div>
              ))}
            </div>

            {/* Bolivia badge */}
            <div className="mt-8 pt-6 border-t border-[#2A2A2A] flex items-center gap-3">
              <span className="text-2xl">🇧🇴</span>
              <div>
                <p className="text-white text-sm font-semibold">Distribuidor Oficial Bolivia</p>
                <p className="text-gray-600 text-xs">Envíos a todo el país desde La Paz</p>
              </div>
            </div>
          </div>
        </div>
